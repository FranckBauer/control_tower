...
rsync -az --exclude 'venv' --exclude '__pycache__' --exclude '.git' --exclude 'node_modules' \
  ...
  "$PROJECT_DIR/" franck@192.168.1.16:~/perso/control_tower/
echo "  OK"
...
ssh -o ConnectTimeout=5 franck@192.168.1.16 "pkill -f 'uvicorn agent' 2>/dev/null; sleep 1; cd ~/perso/control_tower && source venv/bin/activate && nohup python3 -m uvicorn agent.main:app --host 0.0.0.0 --port 3001 > /tmp/ct-agent.log 2>&1 & sleep 3 && curl -s http://localhost:3001/health > /dev/null && echo started" || echo "  WARN: SSH failed, Pi may be offline"
echo "  OK"
